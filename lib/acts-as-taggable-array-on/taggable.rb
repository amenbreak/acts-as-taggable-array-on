  ...
  module Taggable
    ...
    module ClassMethod
      def acts_as_taggable_array_on(*tag_def)
        ...
        scope :"with_any_#{tag_name}", ->(tags){ where("#{tag_name} && ARRAY[?]::text[]", parser.parse(tags)) }
        scope :"with_all_#{tag_name}", ->(tags){ where("#{tag_name} @> ARRAY[?]::text[]", parser.parse(tags)) }
        scope :"without_any_#{tag_name}", ->(tags){ where.not("#{tag_name} && ARRAY[?]::text[]", parser.parse(tags)) }
        scope :"without_all_#{tag_name}", ->(tags){ where.not("#{tag_name} @> ARRAY[?]::text[]", parser.parse(tags)) }
